...
function preload() {
    ...
    this.load.spritesheet('dude', 'Assets/astro2.png', {
        frameWidth: 155,
        frameHeight: 161
    });
}
...
function create() {
    ...
    backButton = this.add.image(50, 50, 'back')
        .setInteractive({ useHandCursor: true })
        .setDepth(11)
        .setScrollFactor(0)
        .setScale(0.25)
    ...
    player.lastDirection = 'right'; // Menambahkan properti untuk menyimpan arah terakhir, default ke kanan
    player.setBounce(0.14);
    player.setCollideWorldBounds(true);
    player.body.setEnable(false);
    player.setVisible(false);
    player.setScale(0.6);
    player.body.setSize(85, 125);
    player.body.setOffset(40, 35);

    this.anims.create({ key: 'left', frames: this.anims.generateFrameNumbers('dude', { start: 0, end: 3 }), frameRate: 10, repeat: -1 });
    this.anims.create({ key: 'idle_right', frames: [{ key: 'dude', frame: 5 }], frameRate: 20 });
    this.anims.create({ key: 'right', frames: this.anims.generateFrameNumbers('dude', { start: 6, end: 9 }), frameRate: 10, repeat: -1 });
    this.anims.create({ key: 'idle_left', frames: [{ key: 'dude', frame: 4 }], frameRate: 20 });
...
function loadLevel(levelNumber) {
    ...
    if (currentLevel <= maxLevel) { // Hanya buat bintang jika bukan level setelah menang
        ...
        newStars.children.iterate((child) => {
